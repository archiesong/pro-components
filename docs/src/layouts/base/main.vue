...
const pageTurning = computed(() => {
  const menus = siderMenus.value.reduce(
    (pre, current) => {
      if (current.children) {
    ...
    <a-col :xxl="20" :xl="19" :lg="18" :md="18" :sm="24" :xs="24">
      ...
      <article class="mt--16px" :class="[isMobile ? 'px-16px' : direction === 'ltr' ? 'pl-48px pr-164px' : 'pr-48px pl-164px']">
        ...
        <div class="ant-doc-main-section-pageTurning">
          <RouterLink :to="pageTurning.prevPath" class="font-size-sm font-400">
            <LeftOutlined :class="[pageTurning.prev ? '' : 'hidden']" class="color-#999 font-size-[--ant-font-size-icon] relative line-height-0 align-middle transition-[inset-inline-end] duration-(--ant-motion-duration-slow)" />
            {{ (pageTurning.prevLocale || '').split(' ')[0] }}
            <span class="ms-[--ant-margin-sm] font-400 opacity-[0.8]">{{ (pageTurning.prevLocale || '').split(' ')[1] }}</span>
          </RouterLink>
          <RouterLink v-show="pageTurning.next" :to="pageTurning.nextPath" class="font-size-sm font-400">
            {{ (pageTurning.nextLocale || '').split(' ')[0] }}
            <span class="ms-[--ant-margin-sm] font-400 opacity-[0.8]">{{ (pageTurning.nextLocale || '').split(' ')[1] }}</span>
            <RightOutlined
              class="color-#999 font-size-[--ant-font-size-icon] ms-[1em] relative m-b-[1px]
            transition-[inset-inline-start] duration-(--ant-motion-duration-slow)"
            />
          </RouterLink>
        </div>
      </article>
