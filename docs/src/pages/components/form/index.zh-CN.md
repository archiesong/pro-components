...
- 如果想要设置默认值，请使用 `model`，任何直接使用组件 `value` 和 `onChange` 的方式都有可能导致值绑定失效
...
import { ProForm, ProFormText, ProFormSelect } from '@antdv-next/pro-form'
import { FormItem } form '@antdv-next/pro-utils;'
</script>
<template>
  <!-- 设置整体默认值 -->
  <ProForm :model="obj" />
...
import { ProForm } from '@antdv-next/pro-form'
</script>
...
```vue
<script setup lang="ts">
import { ProFormDependency, ProFormText } from '@antdv-next/pro-form'
</script>
...
```vue
<script setup lang="ts">
import { ProFormText } from '@antdv-next/pro-form'
</script>
