...
>()
map.set('video', (item) => {
  const { children } = item as { children: VideoHTMLAttributes }
  return h('video', { ...children, controls: true, style: { width: '100%' } })
})
...
map.set('list', (item) => {
  ...
  return h('div', null, [
    h('h3', { style: { margin: '8px 0' } }, listConfig.title),
    h('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px' } }, listConfig.children.map((child, index) => h(
      'div',
      {
        key: index,
      },
      h(TypographyText, null, () => child.href ? h(TypographyLink, { href: child.href }, () => child.title) : child.title),
    ))),
  ])
})
</script>
          ...
          children: [
            {
              ...
              children: [
                ...
                {
                  valueType: 'text',
                  children: '节点。并将各自的摩斯计算节点、子账号等的版本信息、业务需求、数据量级（几行几列）等信息同步给到摩斯产运负责人。',
                },
                {
                  valueType: 'image',
                  children: {
