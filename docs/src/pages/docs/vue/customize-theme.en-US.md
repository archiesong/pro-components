...
ProComponents of Vue allows you to customize our design tokens to satisfy UI diversity from business or brand requirements, including primary color, border radius, border color, etc.
...
```stackblitz {title="Customize Design Token"}
<template>
  <a-config-provider
    :theme="{
      token: {
...
We provide `zeroRuntime` mode to further improve application performance. When enabled, ProComponents of Vuents of Vue will no longer generate component styles at runtime, so you need to manually import the style files.
...
```stackblitz {title="Consume Design Token"}
<template>
  <div
    :style="{
...
Normally we recommend using atomic CSS to consume Design Token. We provide a UnoCSS plugin to make it easier to use Design Token.
...
<template>
  <div class="a-c-primary">
    Consume Design Token with Atomic CSS
  </div>
  <div class="c-primary">
    Consume Design Token with Atomic CSS (without prefix)
  </div>
</template>
```
