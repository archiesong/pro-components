...
ProComponents of Vue 设计规范和技术上支持灵活的样式定制，以满足业务和品牌上多样化的视觉需求，包括但不限于全局样式（主色、圆角、边框）和指定组件的视觉定制。
...
我们提供了 `zeroRuntime` 模式来进一步提升应用性能。开启后，ProComponents of Vuents of Vue 将不再在运行时生成组件样式，所以需要自行引入样式文件。
