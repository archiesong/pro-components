  ...
  setup(props) {
    ...
    return () => {
      ...
      return (
        ...
        <details
          class="markdown"
          open={defaultOpen.value || process.env.NODE_ENV !== "production"}
        >
          <summary>
                  ...
                  if (column.key === "value") {
                    const isColor =
                      ...
                      (record.value.startsWith("#") ||
                        record.value.startsWith("rgb"));
                    if (isColor) {
                      return (
                        <ColorChunk color={record.value}>
                          {record.value}
                        </ColorChunk>
                      );
                    }
  ...
  setup(props) {
    ...
    const mergedTokens = computed(() => {
      ...
      component.value.split(",").forEach((comp) => {
        const {
          global: globalTokens = [],
          component: singleComponentTokens = [],
        } = tokenData[comp] || {};
    ...
    return () => {
      return (
        <>
          ...
          <SubTokenTable
            title="Global Token"
            tokens={mergedTokens.value.mergedGlobalTokens}
          />
        </>
      );
    };
