...
export const ProFieldBadgeColor: FunctionalComponent<
  BadgeProps,
  {},
  {
    default?: () => VNode
  }
> = ({ color, ...rest }, { slots }) => <Badge {...rest} color={color} text={slots.default?.()} />

export function objectToMap(value?: ProFieldValueEnumType): ProSchemaValueEnumMap {
...
const TableStatus: {
  ...
  >
  Processing: FunctionalComponent<
    BadgeProps,
    {},
    {
      default?: () => VNode
  ...
  Processing: (props, { slots }) => <Badge {...props} status="processing" text={slots.default?.()} />,
...
type ProFieldStatusType = 'success' | 'warning' | 'error' | 'default' | 'processing' | 'Success' | 'Error' | 'Processing' | 'Default' | 'Warning'
 ...
 * @param key
 */
export function proFieldParsingText(text: SelectValue | { title?: string, value: any } | ({ title?: string, value: any })[], valueEnumParams: ProFieldValueEnumType, key?: number | string): VueNode {
  ...
  if (!valueEnum.has(text as Key) && !valueEnum.has(`${text}`)) {
    return (text as { label: string })?.label || (text as { title: string })?.title || (text as Key)
  }
  ...
  if (!domText) {
    return <Fragment key={key}>{(text as { label: string })?.label || (text as { title: string })?.title || text}</Fragment>
  }
