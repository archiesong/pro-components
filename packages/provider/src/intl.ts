...
/** 将 `zh_CN` 转为 `zh-CN`。 */
function toDashKey<K extends LocaleUnderscoreKey>(k: K) {
  return k.replace('_', '-') as LocaleDashKey
}
...
export function useIntl(): ComputedRef<IntlType> {
  ...
  return computed(() => {
    if (proProvide.value.intl && proProvide.value.intl?.locale !== 'default') {
      return proProvide.value.intl
    }
