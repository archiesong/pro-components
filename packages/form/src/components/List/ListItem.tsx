import type { FormListFieldData, FormListOperation, FormListProps, SearchTransformKeyFn, VueNode } from '@antdv-next/pro-utils'
...
import { childrenToArray, isSpecialNode, normalizeProps, useState } from '@antdv-next/pro-utils'
import { classNames, get, set } from '@v-c/util'
import { Row, Tooltip, useConfig } from 'antdv-next'
import { cloneVNode, computed, defineComponent, isVNode, onUnmounted, ref, shallowReactive, shallowRef, toRefs } from 'vue'
...
const ProFormListItem = defineComponent<ProFormListItemProps, {}, string, CustomSlotsType<ProFormListItemSlots>>((props, { slots }) => {
  ...
  const defaultActionDom = computed(() =>
    [copyIcon.value, deleteIcon.value, upIcon.value, downIcon.value].filter(item => item !== null && item !== undefined),
  )
