...
import { useState } from '@antdv-next/pro-utils'
import { merge } from '@v-c/util'
import { Modal } from 'antdv-next'
import { useConfig } from 'antdv-next/dist/config-provider/context'
import { cloneVNode, defineComponent, shallowRef, Teleport } from 'vue'
...
const ProModalForm = defineComponent(<T extends Record<string, any>, U extends Record<string, any>>(props: ProModalFormProps<T, U>, { expose, slots, attrs }: SetupContext<{}, CustomSlotsType<{
  default?: () => VueNode
}>>) => {
    ...
    const submitterConfig = rest.submitter === false ? false : merge(
      {
        searchConfig: {
          submitText:
            modalProps?.okText ?? config.value.locale?.Modal?.okText ?? '确认',
          resetText:
            modalProps?.cancelText
            ?? config.value.locale?.Modal?.cancelText
            ?? '取消',
        },
        resetButtonProps: {
          preventDefault: true,
  ...
  name: 'ProModalForm',
  inheritAttrs: false,
  props: ['autoComplete', 'autoFocusFirstInput', 'classes', 'clearOnDestroy', 'colProps', 'colon', 'dateFormatter', 'disabled', 'extraUrlParams', 'feedbackIcons', 'formKey', 'grid', 'isKeyPressSubmit', 'labelAlign', 'labelCol', 'labelWrap', 'layout', 'loading', 'modalProps', 'model', 'name', 'omitNil', 'onFieldsChange', 'onFinish', 'onFinishFailed', 'onInit', 'onLoadingChange', 'formRef', 'onOpenChange', 'onReset', 'onSubmit', 'onValidate', 'onValuesChange', 'open', 'params', 'prefixCls', 'preserve', 'readonly', 'request', 'requiredMark', 'rootClass', 'rowProps', 'rules', 'scrollToFirstError', 'size', 'styles', 'submitTimeout', 'submitter', 'syncToModel', 'syncToUrl', 'syncToUrlAsImportant', 'title', 'tooltip', 'trigger', 'validateMessages', 'validateOnRuleChange', 'validateTrigger', 'variant', 'width', 'wrapperCol'],
})
