...
export function getKeyByPath(item: MenuDataItem) {
  const { path } = item
  if (!path || path === '/') {
 ...
 * @param parentTitle
 */
function getItemLocaleName(item: MenuDataItem, parentTitle: string): string | false {
  const { meta } = item
 ...
 * @param parent
 */
function formatter(props: FormatterProps, parent: Partial<MenuDataItem> = { path: '/' }): MenuDataItem[] {
...
// 重要的转换
function bigfishConversions(route: MenuDataItem, props: FormatterProps) {
 ...
 */
function filterMenuData(menuData: MenuDataItem[] = []): MenuDataItem[] {
  return menuData
    .filter(
      (item: MenuDataItem) =>
 ...
 * @param menuData 菜单配置
 */
function getBreadcrumbNameMap(menuData: MenuDataItem[]): RouteListMap<MenuDataItem> {
 ...
 */
export function transformRoute(routeList: MenuDataItem[], locale?: boolean, formatMessage?: (message: MessageDescriptor) => string | undefined): {
