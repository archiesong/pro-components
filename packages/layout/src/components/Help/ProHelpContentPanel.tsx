...
import { useDebounceFn, useEffect } from '@antdv-next/pro-utils'
...
export interface ProHelpContentPanelProps {
  /**
   * 控制当前选中的帮助文档
   */
  selectedKey?: string
  parentItem?: ProHelpDataSource
 ...
 */
const ProHelpContentPanel = defineComponent<ProHelpContentPanelProps>((props, { attrs }) => {
  ...
  const renderItem = (item?: ProHelpDataSource<any>['children'][number]) => {
    if (item?.asyncLoad) {
      return (
        <div class={classNames(attrs.class, proConfig.value.hashId)} id={item.title}>
          <AsyncContentPanel
            key={item?.key}
            item={item!}
            onInit={(ref) => {
