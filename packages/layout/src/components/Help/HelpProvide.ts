 ...
 */
interface ProHelpDataSourceContentType {
  /**
   * h1 标题1类型的数据源子项内容。
   */
  h1: string
 ...
 */
export interface ProHelpDataSourceChildren<
  ValueTypeMap = {
    text: string
  },
> {
 ...
 */
export interface ProHelpDataSource<ValueType = 'text'> {
  /**
   * key 数据源项的唯一标识
   */
  key: string
...
export interface ProHelpContextType {
  /**
  ...
  valueTypeMap: Map<
    string,
    (item: ProHelpDataSourceChildren<any>, index: number) => VueNode
  >
  /**
...
/**
 * 这段代码定义了一个名为 ProHelpProvide 的 Vue 上下文对象，并且指定了该上下文对象的初始值为 { dataSource: [] }。
 * 这个上下文对象中包含了一个名为 dataSource 的属性，该属性的值是一个数组，类型为 ProHelpDataSource<any>[]。
 * 该上下文对象通常用于在 Vue 组件树中共享数据，即可以通过在组件中使用 useProHelpProvider，
 * 将 dataSource 和 valueTypeMap 数据源传递给这些组件，这些组件即可从上下文中获取 dataSource 数据源，实现数据的共享和传递。
 */
export const proHelpContextKey: InjectionKey<Ref<ProHelpContextType>> = Symbol('proHelpContext')

export function useProHelpProvider(props: Ref<ProHelpContextType>) {
  return provide(proHelpContextKey, props)
}

export function useProHelpContext() {
  return inject(proHelpContextKey, ref({
    dataSource: [],
    valueTypeMap: new Map(),
  } as ProHelpContextType))
}
