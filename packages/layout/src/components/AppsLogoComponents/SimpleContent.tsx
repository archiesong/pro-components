 ...
 * @param title
 */
function renderLogo(logo: VueNode, title?: VueNode) {
          ...
          if (app?.children?.length) {
            return (
              <div key={index} class={classNames(`${baseClassName}-content-list-item-group`, hashId)}>
                <div class={classNames(`${baseClassName}-content-list-item-group-title`, hashId)}>{app.title}</div>
                <SimpleContent hashId={hashId} itemClick={itemClick} appList={app?.children} baseClassName={baseClassName} />
              </div>
            )
          }
          return (
            <li key={index} class={classNames(`${baseClassName}-content-list-item`, hashId)}>
              <a href={itemClick ? 'javascript:void(0);' : app.url} onClick={() => itemClick?.(app)} target={app.target} rel="noreferrer">
