...
// import { isNeedOpenHash, useProConfig } from '@antdv-next/pro-provider'
...
import { privateSiderMenuProps, siderMenuProps } from '../SiderMenu/siderMenuProps'
import { useStyle } from './style'

export function topNavHeaderProps() {
  return {
    ...siderMenuProps(),
    ...globalHeaderProps(),
...
const TopNavHeader = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    return () => {
      const { contentWidth, layout, class: className, style, onMenuHeaderClick, navTheme, avatarProps } = props
      const actionsRender = getSlot(slots, props, 'actionsRender')
      const appListRender = getSlot(slots, props, 'appListRender')
      return wrapSSR(
        <div
          class={classNames(prefixCls.value, hashId.value, className, {
